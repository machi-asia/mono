...
- Local Ollama (qwen) fallback for the Rose agent so local development is token-free (ADR 013):
  - New `packages/rose/src/agent/ollamaClient.ts`: `shouldUseOllamaProvider` (auto = `!process.env.VERCEL_URL`, overridable via `OLLAMA_ENABLED=true/false/auto`), `resolveOllamaBaseUrl` (`OLLAMA_BASE_URL` or `http://localhost:11434`), `getOllamaModelLabel` (auto-detect first `qwen*` tag from `/api/tags`, fallback `OLLAMA_MODEL`, then `qwen2.5:3b`), `toOllamaMessages` (flattens Gemini conversation parts incl. tool calls/responses into chat text and appends the JSON tool-call hint), plus `callOllamaChat`, `callOllamaStream`, `callOllamaMessagesChat`, `callOllamaMessagesStream`.
  - `agentRunner.ts` (stream + non-stream paths) branches orchestration, Groq-style tool synthesis, and webSearch-fallback synthesis to the Ollama provider in dev; the Gemini key guard is relaxed in Ollama mode; Langfuse shows `ollama:<model>` labels; non-stream orchestrator errors are now surfaced gracefully.
  - `webSearch` falls back to synthetic results generated by the local model when no `SERPAPI_KEY` and Ollama mode is active.
  - Unit tests `ollamaClient.test.ts` covering the provider-detection env matrix, base-URL resolution, and message flattening (65 tests total in `packages/rose`).
- New env configuration keys: `OLLAMA_BASE_URL`, `OLLAMA_MODEL`, `OLLAMA_ENABLED` in `apps/rose/.env.sample` and `apps/docs/.env.sample` (Configuration Keys section).

### Fixed

- Ollama calls now have hard timeouts (10s connect, 60s idle between stream chunks, 180s overall) so a stalled local model degrades to a graceful error callout instead of hanging the UI on "thinking" forever.
- Ollama model label resolution: auto-detection of the installed `qwen*` tag now runs first, with `OLLAMA_MODEL` and the `qwen2.5:3b` default used as fallbacks only. Previously an explicit `OLLAMA_MODEL` skipped detection, so a locally installed `qwen3.5:9b` was ignored and requests failed with `model 'qwen2.5:3b' not found`.
- Ollama requests now pin `num_ctx: 4096` and keep the model warm for 10 minutes (`keep_alive`) to avoid mid-conversation model reloads and context instability.
- Tool-loop loop protection: identical tool calls (`name` + args) are deduplicated per conversation turn in both the streaming and non-streaming agent paths — a repeated request gets an "already executed, finalize now" notice instead of re-executing. The Ollama system hint now instructs the model to call each tool at most once per turn, and oversized Ollama histories are trimmed (system + first user message + recent turns, ≤24k chars) so the growing per-loop prefill at the end of a tool call no longer slows the page down.
- Chat streaming is now batched (~80ms flush) in the Rose chat modal instead of re-rendering the full message on every SSE delta, eliminating the O(n²) string/DOM allocation that could OOM the tab ("out of memory 2") on long or multi-tool responses.
- `MarkdownRenderer` (`markdown-inline.tsx`) sanitizes link `href`s to safe schemes (`http`, `https`, `mailto`, `tel`, `#`, relative paths), neutralizing `file://` and `javascript:` URLs — fixes the "Content at … may not load or link to file:///" browser security error and closes an XSS vector.

### Tooling

- Added project-scoped `opencode.json` overriding the Supabase MCP server to the canonical project (`zyatzdkapdqngwyhiqqn.supabase.co`), replacing the global config's `ailxxxnezfdgbtqxjoil` project ref for this repo. With the correct connection restored, migrations `20260912000000_rose_memory_index_map.sql` (from 0.18.0) and the previously unapplied `20260904000001_rose_personalization.sql` were applied to the canonical project.

### Docs

- Added ADR 013 (Local Ollama Fallback for Rose in Development) and updated `docs/ARCHITECTURE.md` (pipeline bullet + ADR list).

## [0.18.0] - 2026-09-12

### Added

- Index-based long-term memory for the Rose AI agent (`1 index = 1 description` per user) superseding the append-only fact list (ADR 012):
  - Four dedicated tools in `@mono/rose`: `learn` (write a new memory index), `recall` (read a full description on demand), `remember` (edit/replace or rename an index), `forget` (delete an index). Registered alongside `webSearch`/`askQuestion` (6 tools total) with updated slash-command help and system instruction.
  - Per-chat-turn context now injects a compact index list (index + ~120-char excerpt) via `memoryContext.ts` instead of up to 10 full memory rows; full details are recalled on demand.
- `@mono/database` index-based memory API: `createMemory`, `getMemoryByIndex`, `updateMemoryByIndex` (with optional `newCategory` rename), `deleteMemoryByIndex`, `listMemoryIndexes`, `normalizeCategory`, `normalizeImportance`. Old `saveMemory`/`listMemories`/`updateMemory`/`deleteMemory` exports removed.
- Shared server-side memory store `@mono/rose` `memory-store.ts` resolving the Supabase secret/client + in-memory fallback for guest/test sessions; `memoryContext.ts` builds the index-list prompt context.
- Settings modal Memories tab rewritten around memory indexes: "Add Memory Index" form, inline edit with rename (`newIndex`), importance dropdowns, duplicate-index detection, and help tooltips; decomposed into co-located `settings-memories.tsx`.

### Changed

- Decomposed `packages/rose/src/server.ts` (539 lines → 12 lines) into `server/chat.ts`, `server/settings.ts`, `server/usage.ts`, `server/transcribe.ts`, keeping the public `@mono/rose/server` API (`handleRoseChat`, `handleRoseSettings`, `handleRoseUsage`, `handleRoseTranscribe`).
- `RoseMemoryRecord` updated to string `id` and text `importance` (`'low' | 'medium' | 'high'`).

### Database

- Added migration `20260912000000_rose_memory_index_map.sql` (applied to canonical project `zyatzdkapdqngwyhiqqn.supabase.co`): converts `rose_memories.importance` from integer to text with `'medium'` default, creates a unique `(user_id, category)` index enforcing one description per index, and adds a CHECK constraint on importance values.

### Docs

- Added ADR 012 (Index-Based Long-Term Memory) and updated `docs/ARCHITECTURE.md` memory section + ADR list.
- Added `machi-supabase` global skill (`C:\Users\machi\.agents\skills\machi-supabase`) that programmatically detects the canonical project URL from env files before any DB change.

## [0.17.3] - 2026-09-05

### Fixed

- Fixed Client Whisper speech transcription hanging in Firefox:
  - Configured `@huggingface/transformers` ONNX WASM backend to use single-threaded execution (`numThreads = 1`) and disable worker proxies (`proxy = false`) in [speechService.ts](file:///d:/Repositories/machi%20asia/mono/packages/rose/src/voice/speechService.ts).
  - Resolves an issue where Firefox would silently hang when initializing the ONNX WASM runtime because `SharedArrayBuffer` is restricted without cross-origin isolation (`COOP`/`COEP`) headers.
